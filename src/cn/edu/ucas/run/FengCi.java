...
import cn.edu.ucas.mapreduce.FenCiMap;
import cn.edu.ucas.mapreduce.NativeBayes;
import cn.edu.ucas.mapreduce.FenCiMap.ModelTrainMapper;
...
public class FengCi {
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
		...
		job2.setJarByClass(FenCiMap.class);
		job2.setMapperClass(ModelTrainMapper.class); // 为job设置Mapper类
		job2.setOutputKeyClass(Text.class); // 为job的输出数据设置Key类
		job2.setOutputValueClass(Text.class); // 为job输出设置value类
		job2.setInputFormatClass(TextInputFormat.class);
		FileInputFormat.addInputPath(job2, new Path(otherArgs[0]));
		FileOutputFormat.setOutputPath(job2, new Path(otherArgs[1]));// 为job设置输出路径
